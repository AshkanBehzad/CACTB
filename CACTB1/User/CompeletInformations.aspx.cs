    ...
    public partial class CompeletInformations1 : System.Web.UI.Page
    {
        ...
        protected void btnNext_Click(object sender, EventArgs e)
        {
            if (Page.IsValid)
            {
                ...
                cmd.CommandText = "UPDATE Members SET ID=@id,FirstName=@fn,LastName=@ln,NationalID=@nid,Email=@email,PhoneNumber=@phone,AltPhoneNumber=@altPhone,Gender=@gen,Fielde_ID=@fid WHERE ID=@mid";
                cmd.Parameters.AddWithValue("@id", txtMemberID.Text);
                cmd.Parameters.AddWithValue("@fn", txtFirstName.Text);
                cmd.Parameters.AddWithValue("@ln", txtLastName.Text);
                cmd.Parameters.AddWithValue("@nid", txtNatioanlID.Text);
